...
def test_parse_success(client, auth_headers):
    ...
    with patch("app.api.parse.parser.parse_content", return_value="Some CV text"):
        with patch(
            "app.api.parse.embedder.generate_embedding", return_value=[0.1, 0.2]
        ):
            with patch(
                "app.api.parse.parser.extract_metadata", return_value=fake_metadata
            ):
                with patch(
                    "app.api.parse.searcher.insert_cv", return_value=None
                ) as mock_insert:
